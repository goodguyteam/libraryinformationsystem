                    ...
                    <table class="table table-striped table-bordered" id="table3">
                        ...
                        @foreach(\Illuminate\Support\Facades\DB::table('transactions')->latest()->get() as $borrowed)
                            <tr>
                                ...
                                <td>{{  $borrowed->student_borrower_id ? date('h:ia', strtotime(date_add(\Carbon\Carbon::make($borrowed->date_borrowed), date_interval_create_from_date_string(\LIS\CirculationDeadlineSetup::where('code', 'stud')->first()->deadline.' days')))) : date('h:ia', strtotime(date_add(\Carbon\Carbon::make($borrowed->date_borrowed), date_interval_create_from_date_string(\LIS\CirculationDeadlineSetup::where('code', 'fac')->first()->deadline.' days')))) }}<br/>{{ $borrowed->student_borrower_id ? date('M d, Y', strtotime(date_add(\Carbon\Carbon::make($borrowed->date_borrowed), date_interval_create_from_date_string(\LIS\CirculationDeadlineSetup::where('code', 'stud')->first()->deadline.' days')))) :  date('M d, Y', strtotime(date_add(\Carbon\Carbon::make($borrowed->date_borrowed), date_interval_create_from_date_string(\LIS\CirculationDeadlineSetup::where('code', 'fac')->first()->deadline.' days')))) }}</td>
                                <td width="100px">
                                    ...
                                    @if($borrowed->date_returned > \Carbon\Carbon::now())<button class="btn btn-danger" data-toggle="modal" data-target="#confirmReturn{{ $borrowed->id }}"><i class="livicon" data-name="sign-in" data-size="20" data-color="#fff" data-hc="#ccc" data-loop="true"></i></button>
                                        @endif
                                </td>
                            </tr>
                        @endforeach
